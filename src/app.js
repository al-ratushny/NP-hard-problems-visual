...
function splitTitleLines(title, maxChars = 18, maxLines = 3) {
...
function yearLayout(visibleTasks) {
  ...
  const layers = years.map((year) => {
    const tasks = byYear.get(year).slice().sort((a, b) => a.title.localeCompare(b.title));
  ...
  for (const layer of layers) {
    ...
    for (let row = 0; row < layer.rowCount; row += 1) {
      ...
      const rowY = layerTop + row * (NODE_H + NODE_ROW_GAP) + NODE_H / 2;
      const rowCount = rowTasks.length;
      const rowWidth = rowCount * NODE_W + (rowCount - 1) * NODE_ROW_GAP;
      const startX = leftPad + (width - rowWidth) / 2 + NODE_W / 2;
      for (let i = 0; i < rowTasks.length; i += 1) {
        const x = startX + i * (NODE_W + NODE_ROW_GAP);
        positions.set(rowTasks[i].id, { x, y: rowY });
      }
    }
...
function drawGraph() {
  ...
  const { positions: pos, viewHeight } = yearLayout(visibleTasks);
  ...
  for (const red of visibleReductions) {
    ...
    if (state.pathReductionIds.has(red.id)) line.classList.add("path-edge");
    if (state.selectedReductionId === red.id) line.classList.add("selected-edge");
    if (highlight.active) {
  ...
  for (const task of visibleTasks) {
    ...
    rect.setAttribute("fill", colorByClass[task.class] || "#b0beca");
